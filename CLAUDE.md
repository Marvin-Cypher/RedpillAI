...
### Frontend v2 Migration & Cleanup Complete (2025-08-06)
- **Modern UI Framework Migration**: Successfully migrated to Next.js 15.1.7 with enhanced Shadcn/UI components
  - **CLEANUP COMPLETE**: Removed legacy `frontend/` directory, renamed `frontend-v2/` to `frontend/`
...
- **Component Architecture Improvements**: Modular, reusable component system
  ...
  - Full-screen MemoViewer with enhanced markdown rendering
...
- **Deal State Management**: Complete integration between companies and dealflow pipeline
  - Companies API enriched with real-time deal status from backend
  - Deal creation workflow from companies page to dealflow pipeline
...
- **Companies**: Crypto/traditional company tracking with enrichment
- **Deals**: VC pipeline (PLANNED → MEETING → RESEARCH → DEAL → TRACK → PASSED → CLOSED)
- **Conversations**: AI chat with project context and debugging chat_ids
...
- `REDPILL_API_KEY` - Primary AI provider
- `OPENAI_API_KEY` - Fallback AI provider
- `COINGECKO_API_KEY` - Market data
- Backend auto-configures for development/production

### AI Service Configuration
- Dual provider setup in `backend/app/services/ai_service.py`
- Automatic fallback chain: Redpill AI → OpenAI → Mock responses
- VC-specialized system prompts with deal context injection
...
- Frontend: TypeScript throughout with proper interface definitions
- API responses use response_model for automatic OpenAPI generation
...
- Backend: Check if pytest or similar is configured
- Frontend: `npm run lint` and `npm run build` to verify changes
- Look for test commands in package.json scripts

## Deployment

- Docker Compose configuration available for full stack
