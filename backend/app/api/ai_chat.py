"""
Consolidated AI Chat API - Service-First, Async-Safe
Merged features from chat.py and ai_chat.py for comprehensive chat functionality.
"""
...
from ..models.deals import Deal, ResearchMemo
...
async def ai_chat(
    ...
    Consolidated from chat.py and ai_chat.py for service-first architecture.
    """
    ...
    if project_id and project_type:
        if project_type == "company":
            ...
            # Get company name - if company doesn't exist, return error
            try:
                company = db.exec(select(Company).where(Company.id == project_id)).first()
                if company:
                    context_name = company.name
                else:
                    # Company doesn't exist - return error instead of trying to create conversation
                    raise HTTPException(
                        status_code=404,
                        detail=f"Company '{project_id}' not found in our database. Please ensure the company exists before starting a conversation."
                    )
            except HTTPException:
                raise  # Re-raise HTTPException
            except Exception as e:
                print(f"Database error when looking up company {project_id}: {e}")
                raise HTTPException(
                    status_code=500,
                    detail=f"Database error when looking up company: {str(e)}"
                )
        elif project_type == "deal":
            conversation_type = ConversationType.DEAL
            deal_id = project_id
            # Get deal name - if deal doesn't exist, return error
            try:
                deal = db.exec(select(Deal).where(Deal.id == project_id)).first()
                if deal:
                    context_name = deal.company_name
                else:
                    # Deal doesn't exist - return error instead of trying to create conversation
                    raise HTTPException(
                        status_code=404,
                        detail=f"Deal '{project_id}' not found in our database. Please ensure the deal exists before starting a conversation."
                    )
            except HTTPException:
                raise  # Re-raise HTTPException
            except Exception as e:
                print(f"Database error when looking up deal {project_id}: {e}")
                raise HTTPException(
                    status_code=500,
                    detail=f"Database error when looking up deal: {str(e)}"
                )
...
async def list_conversations(
    ...
    if current_user:
        statement = statement.where(Conversation.user_id == current_user.id)
...
# ========================
# DEAL-SPECIFIC CHAT FEATURES (from chat.py)
# ========================
...
async def websocket_endpoint(websocket: WebSocket, deal_id: str):
    """WebSocket endpoint for real-time chat - NO-OP (not integrated)."""
    await websocket.accept()
    
    try:
        # Send a no-op message and close
        response = {
            "type": "no_op",
            "message": "WebSocket endpoint not integrated. Please use REST API endpoints.",
            "deal_id": deal_id,
