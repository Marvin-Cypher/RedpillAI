...
import { MemoViewer, useMemos } from '@/components/memos'
import type { Memo as MemoViewerMemo } from '@/components/memos'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function NotesPage() {
  ...
  const [selectedMemo, setSelectedMemo] = useState<MemoViewerMemo | null>(null)
  ...
  useEffect(() => {
    const loadMemos = () => {
      try {
        ...
        // Load unified memos from ai_memos storage
        const unifiedMemos = localStorage.getItem('ai_memos')
        if (unifiedMemos) {
          const memosData = JSON.parse(unifiedMemos)
          allMemos.push(...memosData.map((memo: any) => ({
            id: memo.id,
            title: memo.title,
            content: memo.content,
            date: memo.createdAt || memo.date || new Date().toISOString(),
            author: memo.author || (memo.type === 'ai' ? 'AI Research Assistant' : 'User'),
            projectId: memo.projectId,
            projectName: memo.projectName,
            projectType: memo.projectType || 'open',
            type: memo.type || 'manual',
            chatId: memo.chatId
          })))
        }

        // MIGRATION: Load old memos from previous storage systems
        // Load AI research memos (old system)
        ...
        if (aiMemos) {
          ...
          allMemos.push(...aiMemosData.map((memo: any) => ({
            id: memo.id || `old-ai-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            title: memo.title,
            content: memo.content,
            date: memo.date || memo.createdAt || new Date().toISOString(),
        ...
        // Load memos from individual project storages (old system)
        ...
        storageKeys.forEach(key => {
          try {
            ...
            allMemos.push(...projectMemos.map((memo: any) => ({
              id: memo.id || `old-proj-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
              title: memo.title,
              content: memo.content,
              date: memo.date || memo.createdAt || new Date().toISOString(),
              author: memo.author || (memo.author?.includes('AI') ? 'AI Research Assistant' : 'User'),
              projectId: memo.projectId,
              projectName: memo.projectName,
              projectType: memo.projectType || 'deal',
        ...
        // Migrate old memos to unified storage
        if (allMemos.length > 0) {
          const unifiedFormat = allMemos.map(memo => ({
            id: memo.id,
            title: memo.title,
            content: memo.content,
            type: memo.type,
            ...
            updatedAt: memo.date,
            projectId: memo.projectId,
            projectType: memo.projectType,
            projectName: memo.projectName,
            chatId: memo.chatId
          }))
          
          // Remove duplicates by id
          const uniqueMemos = unifiedFormat.filter((memo, index, self) => 
            index === self.findIndex(m => m.id === memo.id)
          )
          
          localStorage.setItem('ai_memos', JSON.stringify(uniqueMemos))
        }
  ...
  // Convert memo format for MemoViewer
  const convertToMemoViewerFormat = (memo: Memo): MemoViewerMemo => ({
    id: memo.id,
    title: memo.title,
    content: memo.content,
    type: memo.type,
    ...
    createdAt: memo.date,
    updatedAt: memo.date,
    projectId: memo.projectId || 'general',
    projectType: memo.projectType || 'open',
    projectName: memo.projectName || 'General Notes'
  })

  // Handle view memo click (for markdown rendering)
  const handleViewMemo = (memo: Memo) => {
    const memoViewerFormat = convertToMemoViewerFormat(memo)
    setSelectedMemo(memoViewerFormat)
    setMemoViewerOpen(true)
  }

  // Handle edit button click (for simple text editing)
  ...
  const handleSaveEdit = () => {
    ...
    // Save to unified localStorage
    const storedMemos = JSON.parse(localStorage.getItem('ai_memos') || '[]')
    ...
    localStorage.setItem('ai_memos', JSON.stringify(updatedStoredMemos))
  ...
  const handleSaveNewNote = () => {
    ...
    const unifiedMemo = {
      ...
      author: 'User',
      createdAt: newMemo.date,
      updatedAt: newMemo.date,
      projectId: 'general',
      projectType: 'open' as const,
      projectName: 'General Notes'
    }
    storedMemos.unshift(unifiedMemo)
    localStorage.setItem('ai_memos', JSON.stringify(storedMemos))
    ...
    <div className="p-6">
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground border-t pt-4">
                ...
                <div className="flex space-x-2">
                  ...
                  {memo.projectId && memo.projectName && (
                    <Link href={`/portfolio/${memo.projectId}/deal`}>
      ...
      {selectedMemo && (
        <MemoViewer
          ...
          onDelete={(memoId) => {
            const memoToDelete: Memo = {
              id: selectedMemo.id,
              title: selectedMemo.title,
              content: selectedMemo.content,
              date: selectedMemo.createdAt,
              author: selectedMemo.author || 'User',
              projectId: selectedMemo.projectId,
              projectName: selectedMemo.projectName,
              projectType: selectedMemo.projectType as 'company' | 'deal' | 'open',
