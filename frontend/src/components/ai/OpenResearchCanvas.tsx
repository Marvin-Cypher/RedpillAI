...
export function OpenResearchCanvas({ 
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Check if this should trigger approval workflow (like open-research-ANA)
      ...
      if (shouldTriggerApproval) {
        // Generate contextual research plan based on the user's request
        let researchPlan = `I propose to conduct comprehensive research on "${currentInput}" for ${projectName} covering:\n`
        
        if (currentInput.toLowerCase().includes('founding') || currentInput.toLowerCase().includes('history')) {
          researchPlan += `1. Company founding story and background
2. Founder profiles and previous experience
3. Initial funding rounds and early investors
4. Key milestones and pivotal moments
5. Evolution of business model and strategy`
        } else if (currentInput.toLowerCase().includes('market') || currentInput.toLowerCase().includes('cap')) {
          researchPlan += `1. Current market capitalization analysis
2. Historical valuation trends
3. Peer comparison and market positioning
4. Market dynamics and growth drivers
5. Investment outlook and recommendations`
        } else {
          researchPlan += `1. Background and context analysis
2. Market dynamics and competitive landscape
3. Technical/financial deep dive
4. Risk assessment and considerations
5. Investment implications and insights`
        }
  ...
  const handleApproveResearchPlan = async () => {
    ...
    // Extract research sections from the approved plan
    const planLines = approvalFlow.proposedPlan.split('\n').filter(line => 
      line.match(/^\d+\./) // Lines that start with numbers
    )
    
    const researchSections = planLines.map(line => 
      line.replace(/^\d+\.\s*/, '').trim() // Remove number prefix
    )
    
    // Fallback to default sections if extraction fails
