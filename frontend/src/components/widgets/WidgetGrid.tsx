...
export function WidgetGrid({
  ...
  const renderWidget = (widget: Widget) => {
    ...
    if (!WidgetComponent) {
      return (
        <Card key={widget.id} className="p-4 border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20">
    ...
    const handleRefresh = async () => {
      // Default refresh implementation - could trigger data refetch here
    ...
    return (
      <div
        ...
        style={{
          gridColumn: `span ${Math.min(widget.position.w, 12)}`, // Ensure we don't exceed grid columns
