...
export class RedpillAIProvider {
  ...
  async chat(messages: Array<{ role: string; content: string }>, model = "phala/deepseek-r1-70b") {
    try {
  ...
  async streamChat(messages: Array<{ role: string; content: string }>, model = "phala/deepseek-r1-70b") {
    try {
